...
def normalise_input(user_input):
    ...
    normal1 = remove_punct(user_input)
    normal2 = remove_spaces(normal1)
    return normal2.lower()
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        input1 = input("Please choose an exit:")

        # Normalise the input
        user_input = normalise_input(input1)

        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, user_input) is True:
